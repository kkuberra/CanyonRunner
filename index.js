...
const handleKeyDown = function(ship, ctx, canvas) {
  return function(e) {
    keys[e.keyCode] = true;
    if (keys[DIRECTIONS.LEFT]) {
      deltaX -= 5;
    }
    if (keys[DIRECTIONS.UP]) {
      deltaY -= 5;
    }
    if (keys[DIRECTIONS.RIGHT]) {
      deltaX += 5;
    }
    if (keys[DIRECTIONS.DOWN]) {
      deltaY += 5;
    }
    e.preventDefault();
...
class Game {
  ...
  start() {
    ...
    addEventListener(
      "keydown",
      handleKeyDown(this.ship, this.ctx, this.canvas),
      false
    );
    ...
    intervalId = setInterval(() => {
      ...
      for (let i = 0; i < this.canyon.canyonMap.length; i++) {
        ...
        if (this.ship.back + deltaY === i * 10) {
          ...
          if (
            this.ship.left + deltaX <= this.canyon.canyonMap[i][0] ||
            this.ship.right + deltaX >= this.canyon.canyonMap[i][1]
          ) {
            // Game over!
            this.end();
            return;
